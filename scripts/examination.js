...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (role === 'perawat') {
        ...
        switchExaminationTab('consultation'); // Default to consultation for doctors
        displayProblematicPrescriptions(); // Show prescriptions that need review
    }
...
function displayExaminationQueue() {
    ...
    // Show patients relevant to this module
...
function handleConsultationSubmit(e) {
    ...
    if (needsPrescription) {
        // This function from data-integration.js will also update status to 'Menunggu Resep'
        syncPrescriptionToPharmacy(patientId, {
            notes: consultationData.prescriptionNotes,
            items: [] // Structure for detailed items can be added later
        });
    } else {
        // If no prescription, patient goes straight to payment
...
function loadPatientForConsultation(patientId) {
    ...
    if (vitals) {
        display.innerHTML = `...`; // Populate with vitals data as before
        document.getElementById('keluhan_pasien').value = vitals.keluhan_perawat || '';
    } else {
        display.innerHTML = `<div class="empty-state"><p>Data vital dari perawat belum diinput.</p></div>`;
    }
}
...
// This function should be centralized in main.js or data-integration.js
